...
function solve_minimum_units_subproblem!(prob::ClassicHENSProblem; time_limit = 60.0, presolve = true, optimizer = HiGHS.Optimizer, verbose = false)
    ...
    for i in H_set
        for j in C_set
            M[(i,j)] = min(sum(get_contribution(i,k) for k in intervals), sum(get_contribution(j,k) for k in intervals))
        end
    end
    ...
    get_contribution(i, first(intervals)) == R[i, first(intervals)] + sum(Q[i, j, first(intervals)] for j in C_set))
    ...
    R[i, intervals[k-1]] + get_contribution(i, intervals[k]) == R[i, intervals[k]] + sum(Q[i, j, intervals[k]] for j in C_set))

    # Cold side heat balance. 
    @constraint(model, [j in C_set, k in intervals],
    get_contribution(j, k) == sum(Q[i, j, k] for i in H_set))
